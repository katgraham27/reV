...
class RevNrwalCSV(RevNrwal):
    ...
    @classmethod
    def run(cls, gen_fpath, site_data, sam_files, nrwal_configs,
            output_request, fout, save_raw=False, meta_gid_col='gid',
            site_meta_cols=None):
        """Initialize and run the NRWAL analysis object.
        ...
        fout : str
            ...
            len(output) == num_rows in meta).
        save_raw : False
            THIS INPUT IS NOT AVAILABLE WHEN WRITING TO CSV. The `save_raw`
            option is always set to `False`. Therefore, make sure that you do
            not have any references to "input_dataset_name_raw" in your NRWAL
            config. If you would like to manipulate an input dataset, save it
            to a new name and adjust the name in post-processing.
        meta_gid_col : str
            Column label in the source meta data from gen_fpath that contains
            the unique gid identifier. This will be joined to the site_data
            "gid" column
